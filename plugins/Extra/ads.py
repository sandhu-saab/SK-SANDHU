    ...
    try:
        ...
        if "#" not in command_args or len(command_args.split("#")) < 3:
            await message.reply_text(
                "Usage: /set_ads {ads name}#{time}#{photo URL} <a href=https://t.me/OttSandhu/9>Explain</a>"
            )
            return
        ...
        if len(ads_name) > 35:
            await message.reply_text(
                "Advertisement name should not exceed 35 characters."
            )
            return

        if not re.match(r"https?://.+", url):
            await message.reply_text("Invalid URL format. Use a valid Telegram link.")
            return
        ...
        if duration_or_impression[0] == "d":
            ...
            await message.reply_text(
                "Invalid prefix. Use 'd' for duration and 'i' for impression count."
            )
            return
        ...
        if not reply:
            await message.reply_text(
                "Reply to a message to set it as your advertisement."
            )
            return
        if not reply.text:
            await message.reply_text("Only text messages are supported.")
            return

        await mdb.update_advirtisment(
            reply.text, f"{ads_name}", expiry_date, impression_count
        )
        ...
        await message.reply_text(
            f"Advertisement: '{name}' has been set with the stream link: {url}"
        )
...
async def ads(_, message):
    try:
        ...
        if impression == 0:
            ...
            return
        await message.reply_text(
            f"Advertisement: '{name}' has {impression} impressions left."
        )
    except Exception as e:
        await message.reply_text(f"An error occurred: {str(e)}")
...
async def del_ads(client, message):
    try:
        ...
        if current_link:
            ...
            if is_deleted:
                await message.reply(
                    f"Successfully deleted advertisement and ads photo link: {current_link}!"
                )
            else:
                await message.reply(
                    "Advertisement reset, but stream link deletion failed. Stream link not found or something went wrong! Check logs"
                )
        else:
